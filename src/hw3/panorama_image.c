...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    for(j = 0; j < an; ++j){
        ...
        float curMin = l1_distance(a[j].data, b[0].data, (*a).n);
        int bind = 0; // <- find the best match
        for(i = 1; i < bn; i++) {
            float curDistance = l1_distance(a[j].data, b[i].data, (*a).n);
    ...
    for (int an_i = 0; an_i < an; an_i++) {
        int bi_indx = m[an_i].bi;
        int isSeen = seen[bi_indx];
        if (isSeen == 0) {
            seen[bi_indx] = 1;
            m[count] = m[an_i];
            count++;
...
point project_point(matrix H, point p)
{
    ...
    double valY = res.data[1][0] / res.data[2][0];
    double valX = res.data[0][0] / res.data[2][0];
    q.y = (float)(valY);
    q.x = (float)(valX);
    return q;
}
...
{
    float xDist = (p.x - q.x);
    float xxDist = xDist * xDist;
    float yDist = (p.y - q.y);
    float yyDist = yDist * yDist;
    float dist = sqrtf(xxDist + yyDist);
    return dist;
}
...
int model_inliers(matrix H, match *m, int n, float thresh)
{
    ...
    for (i = 0; i < n; i++) {
        point proj = project_point(H, m[i].p);
        float dist = point_distance(proj, m[i].q);
        if (dist <= thresh) {
            match swap = m[count];
            m[count] = m[i];
            m[i] = swap;
...
void randomize_matches(match *m, int n)
{
    ...
    for(i = n - 1; i >= 1; i--){
        j = rand() % (i + 1); //Determine random number i <= j < n
...
matrix compute_homography(match *matches, int n)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        b.data[qMatrix][0] = xp;
        b.data[qMatMatch][0] = yp;
        for(int j = 0; j < 8; j++) {
            M.data[qMatrix][j] = rowTop[j];
            M.data[qMatMatch][j] = rowBot[j];
        }
...
matrix RANSAC(match *m, int n, float thresh, int k, int cutoff)
{
    ...
    for (int i = 0; i < numIterations; i++) {
        ...
        matrix homogenousMat = compute_homography(m, 4);
        if (homogenousMat.data != 0) {
            int curInliners = model_inliers(homogenousMat, m, n, thresh);
            if (curInliners > best) {
                best = curInliners;
                Hb = compute_homography(m, curInliners);
                if(Hb.data != NULL) {
                    if (best > cutoff) {
                    return Hb;
                    }
...
image combine_images(image a, image b, matrix H)
{
    ...
    for(int x = 0; x < w; x++) {
        for (int y = 0; y < h; y++) {
            float xOffset = x + dx;
            float yOffset = y + dy;
            point projection = project_point(H, make_point(xOffset, yOffset));
            if (projection.x >= 0 && projection.y >= 0 && projection.x < b.w && projection.y < b.h) {
                for (int i = 0 ; i < b.c; i++) {
                    float val = bilinear_interpolate(b, projection.x, projection.y, i);
                    set_pixel(c, x, y, i, val);
                }
            }
        }
    }
